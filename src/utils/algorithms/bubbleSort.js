...
export default async function bubbleSort(bars) {
    for (let i = 0; i < bars.length; i++) {
      for (let j = 0; j < (bars.length - 1 - i); j++) {
        //bars[j].changeColor(greenColor);
        //bars[j+1].changeColor(greenColor);
        ...
        //bars[j].changeColor(blueColor);
  
        // if highest value is in the back, change it to orange
        //bars[j+1].changeColor(orangeColor);
      }
    }
    //bars[0].changeColor(orangeColor);
}